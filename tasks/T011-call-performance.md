...
The post-commit Performance Preview at
`74c3a931c799ba1dce4303850f38840f73d0ac75` confirmed a 0.6892x overall
improvement on hosted Linux (31.08% lower wall ns/op), with a 95% confidence
interval of [0.6889x, 0.6994x]. All seven hosted cases improved:
`many_locals_call` was 0.5264x, `captured_write` 0.5663x,
`plain_function_call` 0.6136x, `captured_read` 0.6326x, `method_call` 0.6609x,
`property_read` 0.9779x, and `array_read` 0.9874x. All 21 linearity probes
passed, all three requested blocks were valid, and CI and the full Test262
Coverage workflow were green. The same run reduced candidate/QuickJS-NG from
13.4084x to 8.5696x overall; the remaining hosted case ratios range from
4.2359x for `many_locals_call` to 15.0366x for `property_read`.

Once nested-VM setup was removed, sampling exposed the general bytecode
`HashMap<String, usize>` lookup used to map simple parameters and received
upvalues into a numeric scratch frame. The numeric executor admits at most 32
locals, and these bindings normally occupy its first few slots, so it now uses
a bounded linear name scan instead of SipHash. A three-block comparison with
seed `20251021` measured 0.9178x overall. An independent five-block comparison
with seed `20251022` contained all 70 expected measurements and reproduced
0.9195x overall (8.05% lower wall ns/op). All five affected cases improved:
`captured_read` was 0.8706x, `plain_function_call` 0.8739x,
`captured_write` 0.8848x, `method_call` 0.8971x, and `many_locals_call`
0.9298x. The unrelated cases were `array_read` 0.9936x and `property_read`
0.9961x. These are exploratory local binaries without provenance receipts;
use the post-commit Performance Preview for hosted evidence.

The post-commit Performance Preview at
`d88ec915215cfcb7e3a8f64305f3af67196c982d` measured 0.9895x overall on
hosted Linux, with a 95% confidence interval of [0.9799x, 1.0031x]. The point
estimate agrees with the local improvement, but the interval crosses 1.0, so
the hosted result is directional rather than confirmed. The hosted affected
cases were `captured_read` 0.9277x, `method_call` 0.9686x,
`plain_function_call` 0.9696x, `captured_write` 0.9721x, and
`many_locals_call` 1.0207x. The unrelated cases were `property_read` 1.0300x
and `array_read` 1.0430x. All 21 linearity probes passed, all three requested
blocks were valid, and CI and Test262 Coverage were green. The same run reduced
candidate/QuickJS-NG overall from 8.5696x to 8.2719x.

Sampling the resulting executor showed that numeric binary operations still
round-tripped already-unboxed operands through temporary `Value` objects and
the general VM helper. The numeric leaf path now evaluates the primitive
arithmetic and comparison operations directly as `f64` or boolean scratch
values; less common numeric operators continue to use the existing helper, and
unsupported semantics still fall back transactionally. A three-block
comparison with seed `20251023` measured 0.9722x overall. An independent
five-block comparison with seed `20251024` contained all 70 expected
measurements and reproduced 0.9699x overall (3.01% lower wall ns/op). All five
affected cases improved: `many_locals_call` was 0.9034x,
`plain_function_call` 0.9612x, `captured_write` 0.9715x, `captured_read`
0.9761x, and `method_call` 0.9823x. The unrelated cases were `array_read`
...
The post-commit Performance Preview at
`b43df11ff2693aa290d01a0255579d61244a1e6a` confirmed a 0.9844x overall
improvement on hosted Linux (1.56% lower wall ns/op), with a 95% confidence
interval of [0.9775x, 0.9890x]. `property_read` improved to 0.8730x while the
other hosted cases ranged from 0.9874x to 1.0305x. All 21 linearity probes
passed, all three requested blocks were valid, and CI was green. The preview
remained informational because its three-block cohort is non-claim and the
critical-family precision limit was exceeded. The same run reduced
candidate/QuickJS-NG overall from 7.5057x to 7.3769x.

Computed numeric-literal member reads still materialized a number constant on
the operand stack and rediscovered its array-index form on every execution.
The compiler now emits a dedicated index-get op for valid array-index literals.
Dense arrays and typed arrays consume the immediate index directly; holes,
custom prototypes, ordinary objects, accessors, Proxies, and other receivers
retain the generic property path through the canonical decimal key. A
three-block comparison with seed `20251113` measured 0.9446x overall. An
independent five-block comparison with seed `20251117` contained all 70
expected measurements and reproduced 0.9440x overall (5.60% lower wall ns/op).
`array_read` improved to 0.8816x (11.84% lower wall ns/op); the other cases also
measured below 1.0 in that run: `property_read` 0.9248x, `captured_read`
0.9427x, `captured_write` 0.9487x, `plain_function_call` 0.9576x,
`method_call` 0.9735x, and `many_locals_call` 0.9831x. These are exploratory
local binaries without provenance receipts; use the post-commit Performance
Preview for hosted evidence.
...
0.9265x and `many_locals_call` to 0.9344x. The successful rerun had 3/3 valid
blocks and all 21 linearity probes passed; the first attempt was invalid solely
because the fixed QuickJS-NG `array_read` linearity probe was an outlier. CI and
Test262 Coverage were green. The successful run measured candidate/QuickJS-NG
at 6.8489x overall, ranging from 2.9398x for `many_locals_call` to 12.0835x for
`property_read`.

Caching the immutable direct-leaf eligibility predicate at function creation
looked promising locally: three blocks measured 0.9940x overall and an
independent five-block run measured 0.9890x. Hosted evidence at
`c17d3164cea160eb220d11c8c695c80f6e272f71` rejected it, however:
candidate/base was 1.0020x overall with a 95% confidence interval of [1.0005x,
1.0087x]. `many_locals_call` regressed to 1.0110x and `array_read` to 1.0079x.
All 21 linearity probes passed, all three blocks were valid, and CI was green,
