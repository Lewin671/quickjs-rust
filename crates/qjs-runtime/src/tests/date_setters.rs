...
fn evaluates_date_utc_time_setters() {
    assert_eq!(
        ...
        Ok(Value::String(
            "122645006|1970-01-02T10:04:05.006Z".to_owned().into()
        ))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String(
            "123072013|1970-01-02T10:11:12.013Z".to_owned().into()
        ))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String(
            "173045006|1970-01-03T00:04:05.006Z".to_owned().into()
        ))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String(
            "99005006|1970-01-02T03:30:05.006Z".to_owned().into()
        ))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String(
            "99031032|1970-01-02T03:30:31.032Z".to_owned().into()
        ))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String(
            "97480006|1970-01-02T03:04:40.006Z".to_owned().into()
        ))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String(
            "97480041|1970-01-02T03:04:40.041Z".to_owned().into()
        ))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String(
            "97445500|1970-01-02T03:04:05.500Z".to_owned().into()
        ))
    );
    assert_eq!(
        eval(
...
fn date_setters_coerce_arguments_before_invalid_date_return() {
    assert_eq!(
        ...
        Ok(Value::String("1:true:0".to_owned().into()))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String("month,date:true:0".to_owned().into()))
    );
    assert_eq!(
        eval(
            "var d = new Date(NaN); var calls = 0; \
...
fn date_setters_coerce_optional_arguments_with_environment() {
    assert_eq!(
        ...
        Ok(Value::String(
            "1:2016-03-07T11:36:23.002Z".to_owned().into()
        ))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String(
            "1:2016-07-02T11:36:23.002Z".to_owned().into()
        ))
    );
}
