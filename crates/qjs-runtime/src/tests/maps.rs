...
fn evaluates_map_constructor_and_prototype() {
    ...
    assert_eq!(
        ...
        Ok(Value::String("[object Map]".to_owned().into()))
    );
    assert_eq!(
...
fn map_constructor_uses_prototype_set_adder() {
    assert_eq!(
        ...
        Ok(Value::String("2:a:1|b:2|:true:true:2".to_owned().into()))
    );
...
fn evaluates_map_group_by_arrays() {
    ...
    assert_eq!(
        ...
        Ok(Value::String("2:a:b:a0b1".to_owned().into()))
    );
    assert!(
...
fn evaluates_map_basic_methods() {
    ...
    assert_eq!(
        ...
        Ok(Value::String("true:false:0".to_owned().into()))
    );
    assert_eq!(
...
fn evaluates_map_get_or_insert_methods() {
    ...
    assert_eq!(
        ...
        Ok(Value::String("2:2:1".to_owned().into()))
    );
    assert_eq!(
...
fn evaluates_map_iterators_and_for_each() {
    assert_eq!(
        ...
        Ok(Value::String("false:a:1|b:2|true:true".to_owned().into()))
    );
    assert_eq!(
        eval(
    ...
    assert_eq!(
        ...
        Ok(Value::String("1:false|2:false|true:true".to_owned().into()))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String(
            "ctx:a:1:true|ctx:b:2:true|:true".to_owned().into()
        ))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String("a:1|b:2|c:3|".to_owned().into()))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String("a:1|b:2|a:3|:2".to_owned().into()))
    );
