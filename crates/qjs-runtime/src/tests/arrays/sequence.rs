...
fn evaluates_array_sequence_builtins() {
    ...
    assert_eq!(
        ...
        Ok(Value::String("[object Boolean]".to_owned().into()))
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Ok(Value::String("bc".to_owned().into()))
    );
    assert_eq!(
        eval(
    ...
    assert_eq!(
        ...
        Ok(Value::String("1:0:true".to_owned().into()))
    );
    assert_eq!(
        eval(
    ...
    assert_eq!(
        ...
        Ok(Value::String("0:1:true".to_owned().into()))
    );
    assert_eq!(
        eval(
            "let a = [0]; a.length = 3; let b = a.concat(); b.length + ':' + b.hasOwnProperty('1') + ':' + (b[1] === undefined);"
        ),
        Ok(Value::String("3:false:true".to_owned().into()))
    );
    assert_eq!(
        eval(
    ...
    assert_eq!(
        ...
        Ok(Value::String("4:a:false:c".to_owned().into()))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String("4000:false:false:true".to_owned().into()))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String("4:0:false:true".to_owned().into()))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String("4000:0:3999".to_owned().into()))
    );
    assert_eq!(
        eval(
    ...
    assert_eq!(
        ...
        Ok(Value::String("1||3|1||3:false:false".to_owned().into()))
    );
    assert_eq!(
        eval(
    ...
    assert_eq!(
        ...
        Ok(Value::String("1,4,3,4,5".to_owned().into()))
    );
    assert_eq!(
        eval("[1, 2, 3, 4, 5].copyWithin(-2, 0, 2).join();"),
        Ok(Value::String("1,2,3,1,2".to_owned().into()))
    );
    assert_eq!(
        eval("[1, 2, 3, 4].copyWithin(1, 0, 3).join();"),
        Ok(Value::String("1,1,2,3".to_owned().into()))
    );
    assert_eq!(
        eval(
            "let o = { length: 4, 0: 'a', 2: 'c', 3: 'd' }; Array.prototype.copyWithin.call(o, 1, 2); o[0] + ':' + o[1] + ':' + o[2] + ':' + o[3];"
        ),
        Ok(Value::String("a:c:d:d".to_owned().into()))
    );
    assert_eq!(
        eval(
    ...
    assert_eq!(
        ...
        Ok(Value::String("2:false:1".to_owned().into()))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String("2:9:1".to_owned().into()))
    );
    assert_eq!(
        eval(
    ...
    assert_eq!(
        ...
        Ok(Value::String("false:0:a|1:b|true:true".to_owned().into()))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String("a:true:true".to_owned().into()))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String("0:false|1:false|true:true".to_owned().into()))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String("a:false|b:false|true:true".to_owned().into()))
    );
    assert_eq!(
        eval(
        ...
        Ok(Value::String("0:a:true:true".to_owned().into()))
    );
...
fn evaluates_array_to_reversed() {
    assert_eq!(
        ...
        Ok(Value::String("3,2,1:1,2,3:false".to_owned().into()))
    );
    ...
    assert_eq!(
        ...
        Ok(Value::String("c||a".to_owned().into()))
    );
    assert_eq!(
        eval("Array.prototype.toReversed.call('abc').join('');"),
        Ok(Value::String("cba".to_owned().into()))
    );
    assert_eq!(
...
fn evaluates_array_to_spliced() {
    assert_eq!(
        ...
        Ok(Value::String("1,a,b,4:1,2,3,4:false".to_owned().into()))
    );
    assert_eq!(
        eval("[1, 2, 3].toSpliced(-1, 1, 9).join();"),
        Ok(Value::String("1,2,9".to_owned().into()))
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Ok(Value::String("axc".to_owned().into()))
    );
    assert_eq!(
...
fn evaluates_array_with() {
    assert_eq!(
        ...
        Ok(Value::String("1,9,3:1,2,3:false".to_owned().into()))
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Ok(Value::String("9,2,3".to_owned().into()))
    );
    assert_eq!(
