...
fn compile(bytecode: &Bytecode, header: usize, backedge: usize) -> Option<TypedLoopProgram> {
    ...
    let builder = loop {
        ...
        if discovered_slots.is_empty() && discovered_reads.is_empty() {
            #[cfg(feature = "perf-counters")]
            if std::env::var_os("QJS_TL_TRACE").is_some() {
                eprintln!("TLFAIL region {header}..{backedge}");
            }
            return None;
        }
    ...
    // locals, globals, and constants begin after the entire reservation. Pack
    // those persistent registers immediately after the highest actually used
    // stack register before the runtime sizes its scratch files.
    let register_count = compact_scalar_registers(
        &mut ops,
        &mut site_entries,
        &mut local_slots,
        &mut written_locals,
...
impl<'a> Builder<'a> {
    ...
    fn compile(&mut self) -> Option<()> {
        ...
        while ip <= self.backedge {
            ...
            match self.compile_element_assignment(ip) {
                None => {
                    self.trace_give_up("TLGIVEUP-EA", ip);
                    return None;
                }
                Some(Some(next)) => {
                    ip = next;
            ...
            if self.compile_op(op, ip).is_none() {
                self.trace_give_up("TLGIVEUP", ip);
                return None;
            }
    ...
    /// executor's deoptimization trace, one run over a corpus names every
    /// blocker.
    ...
    /// Whether the callee below the top argument was read as the named
    /// property `name` -- `receiver.name(arg)` about to be called.
    fn callee_is_named_read(&self, name: &str) -> bool {
        let Some(&(_, Class::Boxed, Origin::NamedRead(site))) = self
            .stack
            .len()
            .checked_sub(2)
            .and_then(|depth| self.stack.get(depth))
        else {
            return false;
        };
        matches!(
            self.bytecode.code.get(site as usize),
            Some(Op::GetPropNamed { key, .. }) if key.as_ref() == name
        )
    }
    ...
    fn compile_op(&mut self, op: &Op, ip: usize) -> Option<()> {
        match op {
            ...
            Op::CallResolved(argc) if *argc <= 2 => {
                ...
                if *argc == 1 && receiver_class == Class::Boxed && self.callee_is_named_read("push")
                {
                    // `list.push(item)`: the value may be an object, so it is
                    // taken boxed; the callee is verified at run time.
                    let (value, _) = self.pop_boxed()?;
                    let (callee, _) = self.pop_boxed()?;
                    let (receiver, _) = self.pop_boxed()?;
                    let dst = self.slot_scalar()?;
                    self.emit(TypedOp::ArrayPush {
                        dst,
                        receiver,
                        callee,
                        value,
