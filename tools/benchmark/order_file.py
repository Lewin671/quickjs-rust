...
_PROFILE_LINE = re.compile(r"^[\s+!:|]*(\d+)\s+(.*?)\s+\(in qjs\)")
...
def parse_profile(text: str) -> Counter[str]:
    """Samples per function key, summed over every call-graph occurrence."""
...
def profile_process(argv: list[str], seconds: float) -> Counter[str]:
    with tempfile.TemporaryDirectory(prefix="qjs-order-") as work:
        ...
        process.wait()
        return parse_profile(report.read_text(errors="replace")) if report.exists() else Counter()
